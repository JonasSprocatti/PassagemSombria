...
- **Ataque de arma mirado** (`[data-atq]` em `telaMesa`): quando há combate ativo, o seletor de alvo (combatentes do rastreador) só aparece se `souMestre` **ou** for a vez do personagem (`camp.combate.ordem[turno].personagem_id === meuPers.id`); ao mirar, acerto (`total` vs `alvo.cd`) e dano se resolvem sozinhos e o payload da rolagem ganha `alvo_resolvido` (esconde o botão “🩸 aplicar” do Mestre). `ataquesDoCombatente(c)` deriva ataques de linhas de jogador no rastreador para o Mestre rolar/direcionar por eles.
- **Botão “🩸 aplicar N de dano”** (`.m-aplicar`, Mestre, mensagens de `rolagem` com `dano_total`): se a rolagem carrega `payload.total`, compara com a Defesa do alvo escolhido (`alvo.cd`, ou `10 + manobra` para nave). Rolagem < Defesa (ou `fumble`, e sem `crit`) → `confirmModal` de override; recusando, posta “errou” e não aplica. Ataques sem rolagem de acerto (`payload.total == null`, ex. efeitos com teste de resistência) aplicam direto.
- **`resolve:{tipo:"disputa"}`** (Invasão da Sombra): disputa real de `d20 + atributo` contra um combatente escolhido — personagem usa o atributo real (`calc`), inimigo usa `NIVEIS_AMEACA[ameaca].ordem` (0–6) ou `cd − 10`. Vitória tagueia o alvo com a condição `dominado` (1 turno); derrota causa `derrota.danoProprio`. Marca `usos` (1×/descanso). Fora de combate, cai no fallback Mestre-rola-`+2`.
...
- **`resolve:{tipo:"salvaguarda", atributo, cd?, dado?, cond?, turnos?, area?}`**: Fogo de Supressão (Sab → Amedrontado), Grito de Saqueador (Sab, área → Amedrontado), Sinfonia do Inverno (Con, área, 4d8 → Paralisado). CD do teste = `k.cd` do conjurador.
