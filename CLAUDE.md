...
- **`dcSalvaguarda(k, atributo)`** = `8 + mod` — CD de um teste de resistência forçado por habilidade/palavra-chave do personagem. Usado pela `salvaguarda`. Granadas mantêm o `cd` fixo do item.
...
- **`aurasAtivas()` / `auraNega(oque)`** = auras de criatura (`hab.gatilho === "aura"`) das criaturas vivas no rastreador. Sem posição, valem para todo o combate. `auraNega("tecnomancia")` bloqueia `#conjurar`; `auraNega("cura")` bloqueia `#enviar-cura`. Banner no painel de combate lista todas (inclui `imunidade` e `implantes`, que o Mestre adjudica).
