...
**Cuidado — histórico do chat**: a busca inicial de `mensagens` em `telaMesa` precisa ser `.order("criado_em", { ascending: false }).limit(120)` e **depois inverter o array** (`[...data].reverse()`) antes de guardar em `historico`. Buscar `ascending: true` + `limit(N)` pega as N mensagens mais **antigas**, não as mais recentes — numa campanha com mais de N mensagens (fácil de bater: cada ação de combate gera várias `sistema`), tudo recente some do chat ao recarregar a página, sem erro nenhum (mensagens novas continuam aparecendo na hora via `pintarMsg`, só o rebuild do histórico no load é que fica truncado errado). Já foi bug real uma vez — não reintroduzir.
